(() => {
  ...
  function refreshRecents(){const items=[...document.querySelectorAll('#agent-replies .agent-reply:not(.muted)')].slice(0,5);if(!items.length)return;recentContainer.innerHTML=items.map(reply=>`<div class="drawer-recent"><span class="drawer-recent-dot"></span><div class="drawer-recent-copy"><strong>${String(reply.textContent||'Agent result').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</strong><span>Amosclaud Agent result</span></div></div>`).join('');recentContainer.querySelectorAll('.drawer-recent').forEach(item=>item.addEventListener('click',()=>scrollToTarget('agent-replies')))}
  hydrateUser();setInterval(refreshRecents,1500);
})();