...
function rememberRepository(repository){if(!repository)return;localStorage.setItem('amosclaud-last-repository-id',String(repository.id));localStorage.setItem('amosclaud-last-repository-name',`${repository.owner_name}/${repository.name}`)}

document.getElementById('modal-backdrop')?.addEventListener('click',closeModals);

async function apiRequest(path,options={}){const response=await fetch(path,{credentials:'same-origin',...options,headers:{...(options.body?{'Content-Type':'application/json'}:{}),...(options.headers||{})}});if(response.status===401){window.location.assign('/login');throw new Error('Authentication required')}const text=await response.text();let data=null;try{data=text?JSON.parse(text):null}catch{data={detail:text}}if(!response.ok)throw new Error(data?.detail||`Request failed (${response.status})`);return data}
...
function openWorkspace(repositoryId,name=''){localStorage.setItem('amosclaud-last-repository-id',String(repositoryId));if(name)localStorage.setItem('amosclaud-last-repository-name',name);window.location.assign(`/workspace/${repositoryId}`)}
...
document.getElementById('btn-confirm-repository')?.addEventListener('click',async()=>{const name=document.getElementById('repository-name-input').value.trim();if(!name)return showToast('Repository name is required','error');const license=document.getElementById('repository-license-input').value;const initializeGitignore=document.getElementById('repository-gitignore-input').checked;try{const repository=await apiRequest('/api/v1/repositories',{method:'POST',body:JSON.stringify({name,description:document.getElementById('repository-description-input').value.trim(),visibility:document.getElementById('repository-visibility-input').value,initialize_readme:document.getElementById('repository-readme-input').checked})});if(license!=='none'||initializeGitignore){await apiRequest(`/api/v1/repositories/${repository.id}/initialize-template`,{method:'POST',body:JSON.stringify({license,initialize_gitignore:initializeGitignore,branch:repository.default_branch||'main'})})}closeModals();rememberRepository(repository);showToast(`Repository "${name}" created`,'success');window.location.assign(`/workspace/${repository.id}`)}catch(error){showToast(error.message,'error')}});
