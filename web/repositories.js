...
<article class="repository-card">
  ...
  <div class="repository-actions"><button class="btn-primary" type="button" data-action="new-file" data-repository-id="${repository.id}" data-branch="${escapeHtml(repository.default_branch)}">Add file</button><button class="btn-ghost" type="button" data-action="view-tree" data-repository-id="${repository.id}">Code</button></div>
...
async function showTree(repositoryId){const container=document.getElementById(`repository-tree-${repositoryId}`);container.classList.remove('hidden');container.textContent='Loading files…';try{const entries=await apiRequest(`/api/v1/repositories/${repositoryId}/tree`);container.innerHTML=entries.length?entries.map(entry=>`<div class="tree-row"><span>${entry.type==='directory'?'📁':'📄'}</span><span>${escapeHtml(entry.path)}</span></div>`).join('):'<div class="tree-row muted">Repository is empty.</div>'}catch(error){container.textContent=error.message}}

repositoryGrid?.addEventListener('click',event=>{const button=event.target.closest('button[data-action]');if(!button)return;const repositoryId=button.dataset.repositoryId;if(button.dataset.action==='view-tree'){showTree(repositoryId)}else{document.getElementById('file-repository-id').value=repositoryId;document.getElementById('file-branch-input').value=button.dataset.branch||'main';document.getElementById('file-path-input').value='';document.getElementById('file-content-input').value='';document.getElementById('file-commit-input').value='Create file';openModal('modal-file')}});
...
document.getElementById('btn-confirm-repository')?.addEventListener('click',async()=>{const name=document.getElementById('repository-name-input').value.trim();if(!name)return showToast('Repository name is required','error');try{await apiRequest('/api/v1/repositories',{method:'POST',body:JSON.stringify({name,description:document.getElementById('repository-description-input').value.trim(),visibility:document.getElementById('repository-visibility-input').value,initialize_readme:document.getElementById('repository-readme-input').checked})});closeModals();document.getElementById('repository-name-input').value='';document.getElementById('repository-description-input').value='';showToast(`Repository "${name}" created`,'success');loadRepositories()}catch(error){showToast(error.message,'error')}});
