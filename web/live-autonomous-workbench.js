(() => {
  ...
  function addEvent(title, detail, state = 'active', icon = '•') {
    ...
    item.querySelector('strong').textContent = text(title);
    item.querySelector('p').textContent = text(detail);
  ...
  function extractFiles(data) {
    ...
    const fromLogs = (Array.isArray(data.logs) ? data.logs : []).flatMap(line => String(line).match(/(?:[A-Za-z0-9_.-]+\/)+[A-Za-z0-9_.-]+/g) || []);
  ...
  function extractLinks(data) {
    ...
    (Array.isArray(data.logs) ? data.logs : []).forEach(line => { const found = String(line).match(/https?:\/\/[^\s)]+/g); if (found) values.push(...found); });
    const links = [...new Set(values.map(String))];
  ...
  window.addEventListener('amosclaud:agent-phase', event => {
    ...
    if (detail.index === 3 && /authoriz|approval|write/i.test(detail.note || '')) approvals.innerHTML = '<div class="approval-card"><strong>Protected action</strong><div>Autonomous may only write, commit, open a pull request, or deploy when the selected mode authorizes it.</div></div>';
  });
