...
function setMode(nextMode) {
  ...
  if (mode === 'login') {
    ...
    subtitle.textContent = 'Add the setup key to your authenticator app, then enter its six-digit code.';
...
copySecureKey.addEventListener('click', async () => {
  ...
  showMessage('Setup key copied.', true);
});
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (mode === 'register') {
      ...
      showMessage('Setup created. Save the key and enter the rotating six-digit code.', true);
      return;
    }
