...
class KVCache:
    ...
    @property
    def length(self) -> int:
        return 0 if self.key is None else int(self.key.size(self.sequence_dim))

    def append(self, new_key: Tensor, new_value: Tensor) -> tuple[Tensor, Tensor]:
        if new_key.shape != new_value.shape:
            raise ValueError("new key and value tensors must have the same shape")
        if new_key.ndim < 3:
            raise ValueError("KV tensors must have at least three dimensions")
        if self.key is None:
            key, value = new_key, new_value
        else:
            if self.key.shape[:self.sequence_dim] != new_key.shape[:self.sequence_dim] or self.key.shape[self.sequence_dim + 1:] != new_key.shape[self.sequence_dim + 1:]:
                raise ValueError("new KV tensors are incompatible with the existing cache")
            key = torch.cat((self.key, new_key), dim=self.sequence_dim)
            value = torch.cat((self.value, new_value), dim=self.sequence_dim)
        if self.max_length is not None and key.size(self.sequence_dim) > self.max_length:
...
def apply_kv_cache(new_k: Tensor, new_v: Tensor, past_kv_cache: tuple[Tensor, Tensor] | None = None, sequence_dim: int = 2) -> tuple[Tensor, Tensor]:
