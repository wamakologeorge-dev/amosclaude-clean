...
class ModelRouter:
    ...
    def configure_mode(
        ...
        unknown = [
            name for name in backend_names if name not in self.backends
        ]
        if unknown:
            raise KeyError(
                f"unknown model backends: {', '.join(unknown)}"
            )
    ...
    def choose(
        ...
        candidates = (
            ...
            + self.mode_routes.get(mode, [])
            + list(self.backends)
        )
        visited: set[str] = set()
        healthy_states = {"ok", "ready", "healthy", "available"}
        for name in candidates:
            ...
            if str(health.get("status", "")).lower() in healthy_states:
                return backend
        raise RuntimeError(
            f"no healthy model backend is available for mode {mode!r}"
        )
