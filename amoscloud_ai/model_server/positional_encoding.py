...
class SinusoidalPositionalEncoding(nn.Module):
    def __init__(self, d_model: int, max_length: int = 8192, dropout: float = 0.0) -> None:
        super().__init__()
        if d_model <= 0 or max_length <= 0:
            raise ValueError("d_model and max_length must be greater than zero")
    ...
    def forward(self, x: Tensor, offset: int = 0) -> Tensor:
        ...
        return self.dropout(x + self.table[:, offset : offset + length].to(dtype=x.dtype, device=x.device))
