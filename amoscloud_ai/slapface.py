"""Slapface: public Book continuity plus high-confidence repository secret preflight."""
...
_PROVIDER_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("openai", re.compile(r"\bsk-(?:proj-|svcacct-)?[A-Za-z0-9_-]{32,}\b")),
    ("github", re.compile(r"\b(?:gh[pousr]_[A-Za-z0-9]{36,}|github_pat_[A-Za-z0-9_]{60,})\b")),
    ("stripe", re.compile(r"\b(?:sk_live_|rk_live_)[A-Za-z0-9]{24,}\b")),
    ("slack", re.compile(r"\bxox[baprs]-[A-Za-z0-9-]{30,}\b")),
...
_PLACEHOLDER_MARKERS = (
    ...
    "fake", "redacted", "masked", "xxxx", "test-token", "test_key", "test-key",
)
_DOCUMENTATION_PARTS = {
...
_SKIP_DIRS = {
    ...
    "__pycache__", "node_modules", "venv", ".venv", "dist", "build", ".aicode", ".runtime",
}
...
def _looks_placeholder(value: str) -> bool:
    ...
    if not lowered or any(marker in lowered for marker in _PLACEHOLDER_MARKERS):
        return True
    if lowered.startswith(("${{", "$", "{{")) or "secrets." in lowered or "getenv" in lowered:
        return True
...
def _documentation_context(path: str, line: str = "") -> bool:
    normalized = path.replace("\\", "/").lower()
    parts = {part for part in normalized.split("/") if part}
    filename = normalized.rsplit("/", 1)[-1]
    line_lower = line.lower()
    return (
        bool(parts.intersection(_DOCUMENTATION_PARTS))
        or filename.startswith(("readme", "security", "contributing", "billing"))
        or filename.endswith(".md")
        or any(marker in line_lower for marker in ("example", "sample", "fixture", "placeholder", "fake token"))
    )
...
class Slapface:
    ...
    def _chapter_link(self, chapter_id: str) -> str:
        ...
        return f"/api/v1/book/reader#chapter={chapter['id']}"
    ...
    def scan_text(text: str, *, path: str = "<text>") -> dict[str, Any]:
        ...
        def add(
            *,
            ...
            if _looks_placeholder(value):
                return
            adjusted = confidence
            if _documentation_context(path, line_text):
                adjusted -= 0.14
            ...
            blocking = adjusted >= 0.92
            findings.append(
                {
                    "path": path,
                    "line": line_number,
                    "classification": "high_confidence_secret_exposure" if blocking else "suspicious_secret_like_value",
                    "blocking": blocking,
                    ...
                    "documentation_or_test_context": _documentation_context(path, line_text),
        ...
        return {
            ...
            "findings": findings,
            "rule": "Only high-confidence credential evidence blocks; placeholders and lower-confidence secret-like values warn only.",
        }
    ...
    def scan_workspace(
        ...
        *,
        ...
        tracked = cls._tracked_files(root)
        repository_mode = bool(tracked)
        candidates = tracked if tracked else sorted(path for path in root.rglob("*") if path.is_file())
        findings: list[dict[str, Any]] = []
        scanned = 0
        skipped = 0
        for path in candidates:
            ...
            if any(part in _SKIP_DIRS for part in relative.parts):
                continue
            if not repository_mode and path.name.startswith(".env"):
                skipped += 1
                continue
        ...
        return {
            "workspace": str(root),
            "blocked": bool(blocking),
            "repository_mode": "git-tracked" if repository_mode else "filesystem-fallback",
    ...
    def record_handoff(
        ...
        *,
        ...
        if state.get("status") == "blocked" and current:
            raise SlapfaceError(f"Scope already has unresolved Slapface handoff {current.get('handoff_id')}")
        active = {
            ...
            "required_paths": sorted({str(Path(item)).replace("\\", "/") for item in required_paths if str(item).strip()}),
        ...
        state.setdefault("history", []).append({"event": "blocked", "handoff_id": active["handoff_id"], "at": _utcnow()})
        self._save_state(scope, state)
        return self.status(scope)

    def _ensure_secret_handoff(self, *, scope: str | None, agent_id: str, scan: dict[str, Any], source: str) -> dict[str, Any]:
        state = self._load_state(scope)
        ...
        fingerprints = sorted({str(item.get("fingerprint")) for item in blocking if item.get("fingerprint")})
        handoff_id = "secret-" + hashlib.sha256("|".join(fingerprints).encode("utf-8")).hexdigest()[:16]
        return self.record_handoff(
            scope=scope,
            agent_id=agent_id,
            chapter_id="09",
            next_line="Remove the exposed credential material before resuming repository work.",
            risk="High-confidence credential exposure can grant unauthorized access and spread through commits, logs, artifacts, or deployments.",
            missing_pieces=[
    ...
    def remediation_allowed(self, *, scope: str | None, handoff_id: str | None, mode: str, target_path: str | None = None) -> bool:
        state = self._load_state(scope)
        active = state.get("active_handoff")
    ...
    def preflight(
        ...
        *,
        ...
        if workspace is not None and scan_secrets:
            ...
            if scan.get("blocked"):
                self._ensure_secret_handoff(scope=scope, agent_id=actor, scan=scan, source=source)
        status = self.status(scope)
        if not status["blocked"]:
            return {**status, "objective": objective, "mode": mode, "source": source, "secret_scan": scan, "remediation": False}
        remediation = self.remediation_allowed(scope=scope, handoff_id=handoff_id, mode=mode)
        if remediation:
            return {
                **status,
                "work_allowed": True,
                ...
                "secret_scan": scan,
                "remediation": True,
                "message": "Slapface remediation only: repair and verify the active handoff. The original task remains blocked.",
            }
        return {
            **status,
            "work_allowed": False,
    ...
    def resolve(self, *, scope: str | None, handoff_id: str, change_id: str, actor: str) -> dict[str, Any]:
        state = self._load_state(scope)
        active = state.get("active_handoff")
        if state.get("status") != "blocked" or not isinstance(active, dict):
        ...
        report = next((row for row in reversed(self.book.changes(limit=1000)) if str(row.get("change_id")) == change_id), None)
        if report is None:
            raise SlapfaceError(f"No Book change report exists for change_id {change_id}")
        verification = report.get("verification") or {}
        verification_state = str(verification.get("state") or verification.get("status") or "").strip().lower()
        if verification_state not in _VERIFIED_STATES:
            raise SlapfaceError("Slapface resolution requires a verified/passed/completed Book change report")
        required_paths = {str(item) for item in active.get("required_paths") or []}
        changed_paths = {str(Path(item)).replace("\\", "/") for item in report.get("files_changed") or []}
        if required_paths and not required_paths.issubset(changed_paths):
            missing = sorted(required_paths.difference(changed_paths))
            raise SlapfaceError("Verified change report does not cover required repair paths: " + ", ".join(missing))
        state["status"] = "clear"
