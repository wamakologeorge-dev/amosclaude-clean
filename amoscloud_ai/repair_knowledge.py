...
_PATH_PATTERN = re.compile(r"(?<![A-Za-z0-9_])(?:[A-Za-z]:)?(?:[./\\][A-Za-z0-9_.-]+){2,}(?![A-Za-z0-9_])")
_SIGNAL_PATTERNS = (
...
_STOPWORDS = {
    "amosclaud", "candidate", "changed", "command", "error", "failed", "failure",
    "github", "issue", "output", "repair", "repository", "result", "status", "this",
    "using", "verification", "with", "would",
}
...
def _tokens(value: str) -> set[str]:
    return {
        token for token in re.findall(r"[a-z][a-z0-9_-]{2,}", sanitize(value).lower())
...
def _verification_names(results: Iterable[dict[str, Any]]) -> list[str]:
    return sorted({
        ...
        for item in results if isinstance(item, dict) and item.get("name")
    })[:30]
...
def _default_catalog() -> dict[str, Any]:
    return {
        ...
        "updated_at": _now(),
        "capability": {
            "level": 1, "max_level": MAX_LEVEL, "unique_techniques": 0,
            "successful_reuses": 0, "failed_attempts": 0, "last_unlock_at": None,
        },
        "techniques": [],
    }
...
def default_catalog_path(repository: Path | None = None) -> Path:
    ...
    if value := os.getenv("DATA_DIR", "").strip():
        return Path(value).expanduser() / "amosclaud-storage" / "system" / "repair-memory" / "catalog.json"
    return (repository or Path.cwd()).resolve() / ".amosclaud" / "storage" / "repair-memory" / "catalog.json"
...
class VerifiedRepairMemory:
    ...
    def status(self) -> dict[str, Any]:
        with _locked(self.catalog_path):
            catalog = _load(self.catalog_path)
        return {**catalog["capability"], "catalog": str(self.catalog_path), "updated_at": catalog.get("updated_at")}

    def recall(self, query: str, *, changed_files: Sequence[str] = (), limit: int = 4) -> list[MemoryMatch]:
        ...
        for item in catalog["techniques"]:
            ...
            matches.append(MemoryMatch(
                str(item.get("id") or ""), title, signals, kinds,
                tuple(str(value) for value in item.get("verification", []) if value),
                successes, signal_overlap * 6 + token_overlap * 1.5 + kind_overlap * 2 + min(successes, 20) * 0.1,
            ))
        return sorted(matches, key=lambda item: (-item.score, item.technique_id))[:max(1, min(limit, 10))]
    ...
    def learn_verified(
        self, *, failure_evidence: str, changed_files: Sequence[str],
        verification_results: Sequence[dict[str, Any]], source: str = "", source_run_id: str = "",
    ) -> dict[str, Any]:
        ...
        with _locked(self.catalog_path):
            ...
            existing = next((item for item in techniques if item.get("fingerprint") == fingerprint), None)
            ...
            if novel:
                ...
                techniques.append({
                    "id": technique_id, "fingerprint": fingerprint,
                    "title": sanitize("Verified technique: " + ", ".join(signals[:3] or kinds[:3] or ["verified-repair"]), 240),
                    "signals": signals, "file_kinds": kinds, "verification": verification,
                    "source": sanitize(source, 200), "source_run_id": sanitize(source_run_id, 200),
                    "first_verified_at": now, "last_verified_at": now,
                    "success_count": 1, "reuse_count": 0, "status": "verified",
                })
                ...
                catalog["capability"]["successful_reuses"] = int(catalog["capability"]["successful_reuses"]) + 1
            techniques.sort(key=lambda item: str(item.get("id") or ""))
            _save(self.catalog_path, catalog)
            capability = dict(catalog["capability"])
        return {
            "learned": True, "novel": novel, "technique_id": technique_id,
            "level": capability["level"], "max_level": MAX_LEVEL,
    ...
    def learn_from_reports(
        self, *, failure_log: Path, candidate_report: Path, verification_report: Path,
        source: str = "", source_run_id: str = "",
    ) -> dict[str, Any]:
        ...
        changed_files = [str(item) for item in candidate.get("changed_files", []) if isinstance(item, str) and item]
        ...
        verified = (
            ...
            and bool(changed_files) and bool(results)
        ...
        return self.learn_verified(
            ...
            changed_files=changed_files, verification_results=results,
    ...
    def record_report(self, report: Any, *, source_run_id: str = "") -> dict[str, Any]:
        ...
        if not (verdict == "PASS" and changed and evidence and all(getattr(item, "passed", False) for item in evidence)):
            return {"learned": False, "novel": False, **self.status()}
        problem = "; ".join(
            str(getattr(item, "description", ""))
            for item in getattr(report, "repairs", []) or [] if getattr(item, "changed", False)
        )
        problem += "\n" + "; ".join(f"{getattr(item, 'name', '')}: {getattr(item, 'output', '')}" for item in evidence)
        results = [{"name": str(getattr(item, "name", "verification")), "returncode": 0} for item in evidence]
        return self.learn_verified(
            failure_evidence=problem, changed_files=changed, verification_results=results,
            source="repair-engine", source_run_id=source_run_id,
        )
...
def main() -> int:
    ...
    memory = VerifiedRepairMemory(Path(args.catalog).expanduser() if args.catalog else default_catalog_path())
    if args.command == "init":
        memory.initialize(); result = memory.status()
        ...
        result = {"matches": [asdict(item) for item in matches], "output": args.output, **memory.status()}
    elif args.command == "learn":
        result = memory.learn_from_reports(
            failure_log=Path(args.failure_log), candidate_report=Path(args.candidate_report),
            verification_report=Path(args.verification_report), source=args.source,
