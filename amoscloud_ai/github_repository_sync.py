    ...
    with _db() as db:
        db.execute(
            """UPDATE repositories
               ...
               WHERE id=?""",
            (state, detail[:1000], remote_sha, _now(), _now(), repository_id),
        )
        db.commit()
            ...
            results.append(
                {
                    ...
                    "state": "ignored",
                    "detail": f"Push branch {branch} is not mapped default branch {expected_branch}",
                }
            )
            continue
        ...
        with _repo_lock(repository_id):
            try:
                ...
                if repo.is_dirty(untracked_files=True):
                    detail = "Local workspace has uncommitted changes; automatic pull was blocked"
                    _record(repository_id, "conflict", detail, remote_sha)
                    results.append(
                        {"repository_id": repository_id, "state": "conflict", "detail": detail}
                    )
                    continue
                ...
                try:
                    ...
                    remote.set_url(original_url or _public_remote_url(repository_full_name))
                ...
                if branch not in [head.name for head in repo.heads]:
                    ...
                    results.append(
                        {"repository_id": repository_id, "state": "synced", "detail": detail}
                    )
                    continue
                ...
                if local_ref.hexsha == remote_ref.hexsha:
                    ...
                    results.append(
                        {"repository_id": repository_id, "state": "current", "detail": detail}
                    )
                    continue

                if not repo.is_ancestor(local_ref, remote_ref):
                    detail = "Local history is ahead or diverged; automatic pull was blocked"
                ...
                results.append(
                    {"repository_id": repository_id, "state": "error", "detail": detail}
                )
    return results
