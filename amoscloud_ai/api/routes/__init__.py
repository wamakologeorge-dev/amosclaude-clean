...
# Cloud workspace controls and smart project tools share the canonical
# repository surface so main.py mounts them exactly once under /api/v1.
from amoscloud_ai.api.routes import cloud_workspaces as cloud_workspaces
from amoscloud_ai.api.routes import terminal_tools as terminal_tools

for _module in (cloud_workspaces, terminal_tools):
    for _route in _module.router.routes:
        _keys = _route_keys(_route)
        if _keys & _native_keys:
            continue
...
# The Daily Autonomous Builder and the Autonomous Engineering Workforce share
# the canonical Global Task Router. The workforce route module itself is only a
# control-plane surface; execution hooks are installed lazily when work starts.
from amoscloud_ai.api.routes import autonomy as autonomy
from amoscloud_ai.api.routes import task_router as task_router
from amoscloud_ai.api.routes import workforce as workforce

_task_keys = set()
for _route in task_router.router.routes:
    _task_keys |= _route_keys(_route)
for _module in (autonomy, workforce):
    for _route in _module.router.routes:
        _keys = _route_keys(_route)
        if _keys & _task_keys:
            continue
