...
repositories additionally expose the existing authenticated pull and push flow.
"""
...
class ToolCommitRequest(BaseModel):
    message: str = Field(default="Update from Amosclaud cloud terminal", min_length=1, max_length=200)
...
def _open_repository(repository_id: int) -> Repo:
    try:
        ...
        raise HTTPException(status_code=409, detail="Repository Git storage is unavailable") from exc


def _changed_files(repo: Repo) -> tuple[list[str], list[str]]:
    try:
        lines = repo.git.status("--porcelain=v1", "--untracked-files=all").splitlines()
    except GitCommandError as exc:
        raise HTTPException(status_code=409, detail="Unable to read repository changes") from exc
    paths: list[str] = []
    raw: list[str] = []
...
def _make_targets(path: Path) -> set[str]:
    makefile = next((item for item in (path / "Makefile", path / "makefile") if item.is_file()), None)
    if not makefile:
        return set()
    targets: set[str] = set()
    try:
        for line in makefile.read_text(encoding="utf-8", errors="ignore").splitlines():
            ...
            if target and all(character.isalnum() or character in "_-" for character in target):
                targets.add(target)
    except OSError:
        pass
    return targets
...
def _smart_commands(repository_id: int) -> list[dict[str, str]]:
    ...
    commands: list[dict[str, str]] = []
    _add_command(commands, "status", "Git status", "git status --short --branch", "Show the real branch and working-tree changes.", "git")
    _add_command(commands, "diff", "Review changes", "git diff --stat && git diff", "Review actual uncommitted changes.", "git")
    _add_command(commands, "recent", "Recent commits", "git log --oneline --decorate -12", "Show recent repository history.", "git")
    ...
    for script_name, label, description in (
        ...
        if script_name in scripts:
            _add_command(commands, f"npm-{script_name}", label, f"npm run {script_name}", description, "project")

    make_targets = _make_targets(path)
    for target, label in (("test", "Test"), ("build", "Build"), ("lint", "Lint"), ("dev", "Start dev"), ("run", "Run app")):
        if target in make_targets:
            _add_command(commands, f"make-{target}", label, f"make {target}", f"Run Make target {target}.", "project")

    python_project = any((path / name).is_file() for name in ("pyproject.toml", "requirements.txt", "setup.py", "setup.cfg"))
    if python_project:
        if (path / "tests").exists() or (path / "pytest.ini").is_file():
            _add_command(commands, "pytest", "Test", "python -m pytest -q", "Run Python tests.", "project")
        pyproject = ""
        try:
        ...
        if "[tool.ruff" in pyproject:
            _add_command(commands, "ruff", "Lint", "python -m ruff check .", "Run Ruff checks.", "project")
            _add_command(commands, "ruff-format", "Format", "python -m ruff format .", "Format Python files with Ruff.", "project")
        if (path / "manage.py").is_file():
            _add_command(commands, "django", "Run app", "python manage.py runserver 0.0.0.0:8000", "Start the Django development server.", "project")

    if (path / "Cargo.toml").is_file():
        _add_command(commands, "cargo-test", "Test", "cargo test", "Run Rust tests.", "project")
        _add_command(commands, "cargo-build", "Build", "cargo build", "Build the Rust project.", "project")
        _add_command(commands, "cargo-clippy", "Lint", "cargo clippy --all-targets --all-features", "Run Clippy.", "project")
    if (path / "go.mod").is_file():
        _add_command(commands, "go-test", "Test", "go test ./...", "Run Go tests.", "project")
        _add_command(commands, "go-build", "Build", "go build ./...", "Build Go packages.", "project")
    if (path / "docker-compose.yml").is_file() or (path / "compose.yml").is_file():
        _add_command(commands, "compose-config", "Check Compose", "docker compose config", "Validate Docker Compose configuration.", "project")
...
def commit_project_changes(
    ...
    with _repo_lock(repository_id):
        ...
        branch = _safe_branch(body.branch or active or row["default_branch"] or "main")
        ...
        if not active or active != branch:
            try:
                ...
                raise HTTPException(status_code=409, detail=f"Branch '{branch}' is unavailable") from exc
        if not changed:
...
def pull_project_changes(
    ...
    if not _value(row, "github_full_name"):
        raise HTTPException(status_code=409, detail="This is an Amosclaud-native repository; there is no GitHub remote to pull")
    from amoscloud_ai.api.routes.github_repositories import GitHubSyncRequest, pull_github_repository

    return pull_github_repository(
        repository_id,
        GitHubSyncRequest(branch=body.branch, commit_message=body.commit_message),
        user,
    )
...
def push_project_changes(
    ...
    row = _repository(repository_id, int(user["id"]))
    if not _value(row, "github_full_name"):
        raise HTTPException(status_code=409, detail="Amosclaud-native repository changes are already stored on the platform")
    from amoscloud_ai.api.routes.github_repositories import GitHubSyncRequest, push_github_repository
