...
def create_key(body: KeyCreateRequest, request: Request) -> dict:
    ...
    with _connect() as db:
        ...
        cursor = db.execute(
            ...
            (user["id"], body.name.strip(), prefix, _hash(raw), now),
        )
        db.commit()
    return {
        "id": cursor.lastrowid,
        ...
        "prefix": prefix,
        "created_at": now,
        "warning": "Copy this key now. Amosclaud stores only its hash and cannot display it again.",
    }
...
def rotate_key(key_id: int, request: Request) -> dict:
    ...
    with _connect() as db:
        _schema(db)
        row = db.execute(
            "SELECT id,name FROM autonomous_api_keys WHERE id=? AND user_id=? AND revoked_at IS NULL",
            (key_id, user["id"]),
        ).fetchone()
        if not row:
        ...
        cursor = db.execute(
            "INSERT INTO autonomous_api_keys(user_id,name,prefix,key_hash,created_at) VALUES (?,?,?,?,?)",
            (user["id"], row["name"], prefix, _hash(raw), now),
        )
        db.commit()
    return {"id": cursor.lastrowid, "name": row["name"], "key": raw, "prefix": prefix, "created_at": now, "warning": "The previous key was revoked. Copy this replacement now."}
...
def revoke_key(key_id: int, request: Request):
    ...
    with _connect() as db:
        _schema(db)
        updated = db.execute(
            "UPDATE autonomous_api_keys SET revoked_at=? WHERE id=? AND user_id=? AND revoked_at IS NULL",
            (now, key_id, user["id"]),
        )
        db.commit()
    ...
    with _connect() as db:
        ...
        row = db.execute(
            """SELECT users.id,users.name,users.email,users.is_admin,users.provider,autonomous_api_keys.id AS key_id
               FROM autonomous_api_keys JOIN users ON users.id=autonomous_api_keys.user_id
               WHERE autonomous_api_keys.key_hash=? AND autonomous_api_keys.revoked_at IS NULL""",
            (_hash(raw),),
        ).fetchone()
        if row:
            db.execute("UPDATE autonomous_api_keys SET last_used_at=? WHERE id=?", (datetime.now(timezone.utc).isoformat(), row["key_id"]))
            db.commit()
        return row
