(() => {
  ...
  const trusted = byId("trusted-role");
  const preview = byId("command-preview");
  const warning = byId("role-warning");
  const issueList = byId("issue-list");
  const timeline = byId("command-timeline");
  const prForm = byId("pr-form");
  const prRepository = byId("pr-repository");
  const prCustomWrap = byId("pr-custom-wrap");
  const prCustomRepository = byId("pr-custom-repository");
  const prBase = byId("pr-base");
  const prHead = byId("pr-head");
  const prWarning = byId("pr-warning");
  ...
  function commandText() {
    ...
    return ["triage", "health"].includes(type.value) ? base : (objective ? `${base} ${objective}` : base);
  }
  ...
  function updatePreview() {
    ...
    warning.textContent = restricted
      ? (trusted.checked
        ? "The Bot will independently verify your GitHub role and every approval, Doctor, and verification gate."
        : "Restricted request: acknowledge the role requirement before continuing. GitHub enforcement still happens inside the Bot.")
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (restricted && !trusted.checked) {
      warning.textContent = "Confirm the trusted-role acknowledgement before preparing this request.";
    ...
    const issueBody = [
      ...
      `**Execution target:** ${engineRepository}`,
      "",
      ...
      "",
      restricted
        ? "> This request may change repository state. Amosclaud must verify trusted GitHub authority and all approval and verification gates."
  ...
  function validBranch(value) {
    return value.length > 0 && value.length <= 120 && !/[\s~^:?*\[\\]/.test(value) && !value.includes("..") && !value.startsWith("/") && !value.endsWith("/");
  }

  function updatePrRepository() {
    const custom = prRepository.value === "custom";
    prCustomWrap.hidden = !custom;
    prCustomRepository.required = custom;
    prWarning.classList.remove("restricted");
    prWarning.textContent = "The branches must already exist on GitHub. The page opens GitHub’s official compare screen and never creates or merges a PR directly.";
  }

  prRepository.addEventListener("change", updatePrRepository);
  prForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const repository = prRepository.value === "custom" ? prCustomRepository.value.trim() : prRepository.value;
    const base = prBase.value.trim();
    const head = prHead.value.trim();
    if (!validRepository(repository)) {
      prWarning.textContent = "Enter the repository as owner/name.";
      prWarning.classList.add("restricted");
      prCustomRepository.focus();
      return;
    }
    if (!validBranch(base) || !validBranch(head)) {
      prWarning.textContent = "Enter valid existing GitHub branch names without spaces or unsafe ref characters.";
      prWarning.classList.add("restricted");
      (!validBranch(base) ? prBase : prHead).focus();
      return;
    }
    if (base === head) {
      prWarning.textContent = "The base branch and head branch must be different.";
      prWarning.classList.add("restricted");
      prHead.focus();
      return;
    }
    window.location.href = `https://github.com/${repository}/compare/${encodeURIComponent(base)}...${encodeURIComponent(head)}?expand=1`;
  });

  function planIssueUrl(plan) {
    const issueBody = [
      "## Amosclaud package and installation request",
      "",
      `**Selected package:** ${plan.name}`,
      `**Plan type:** ${plan.price}`,
      "",
      "I want to install the Amosclaud GitHub App on repositories I explicitly approve.",
      "",
      "### Required safeguards",
      "- GitHub App installation instead of a pasted personal access token",
      "- Access limited to repositories selected in GitHub",
      "- Minimum permissions",
      "- Short-lived installation tokens only on a secure backend",
      "- Revocable from GitHub at any time",
      "",
      "> This records interest only. Billing and installation activate after the GitHub App and payment service are registered.",
    ].join("\n");
  ...
  function renderPlans() {
    ...
    plans.forEach((plan) => {
      ...
      card.className = `plan-card${plan.featured ? " plan-featured" : ""}`;
      ...
      plan.features.forEach((feature) => { const item = document.createElement("li"); item.textContent = feature; list.append(item); });
      ...
      card.append(badge, heading, price, description, list, link);
      grid.append(card);
    });
  ...
  async function githubJson(url) {
    ...
    if (!contentType.toLowerCase().includes("application/json")) throw new Error("GitHub returned a non-JSON response");
  ...
  async function loadTimeline() {
    try {
      ...
      timeline.replaceChildren(...events.map((entry) => {
        const card = document.createElement("a"); card.className = `timeline-item ${statusClass(entry.status)}`; card.href = entry.url; card.target = "_blank"; card.rel = "noreferrer";
        const marker = document.createElement("span"); marker.className = "timeline-marker"; marker.textContent = entry.status === "PASS" ? "✓" : entry.status === "FAIL" ? "!" : "●";
        const content = document.createElement("span"); const heading = document.createElement("strong"); heading.textContent = `${entry.status} · ${entry.action}`; const meta = document.createElement("small"); meta.textContent = `${entry.request} → ${entry.target}`; content.append(heading, meta);
        const author = document.createElement("span"); author.className = "timeline-author"; author.textContent = entry.author;
        card.append(marker, content, author);
        return card;
      }));
    } catch (_) {
      timeline.innerHTML = '<p class="loading">Live command activity is temporarily unavailable. Open issue #6 directly on GitHub.</p>';
    }
  }
  ...
  async function loadIssues() {
    try {
      ...
      issueList.replaceChildren(...items.map((item) => {
        ...
        const content = document.createElement("span"); const heading = document.createElement("strong"); heading.textContent = item.title; const detail = document.createElement("small"); detail.textContent = `Opened by ${item.user?.login || "GitHub user"} · ${item.comments} comments`; content.append(heading,detail);
        const state = document.createElement("span"); state.className = "issue-state"; state.textContent = "OPEN";
      ...
      issueList.innerHTML = '<p class="loading">Live issues are temporarily unavailable. Open Amosclaud1 Issues directly.</p>';
    }
  }
