...
# Max tokens for the fix response; large files may need more – override via env.
CLAUDE_FIX_MAX_TOKENS = int(os.environ.get("CLAUDE_FIX_MAX_TOKENS", "8192"))
...
def claude_fix_file(file_path: str, file_content: str, request: str) -> str:
    ...
    resp = client.messages.create(
        model=CLAUDE_MODEL,
        max_tokens=CLAUDE_FIX_MAX_TOKENS,
        system=_SYSTEM_FIX,
...
def claude_triage(issue_title: str, issue_body: str, comment: str) -> str:
    ...
    resp = client.messages.create(
        model=CLAUDE_MODEL,
        max_tokens=450,  # ~300 words as instructed in the system prompt
...
def run() -> None:
    ...
    # Log only the length of the comment body to avoid leaking potentially sensitive content.
    print(f"[issue-bot] Comment length: {len(COMMENT_BODY)} chars")
    ...
    if fixed_code:
        if fixed_code.strip() == file_content.strip():
            print(
                "[issue-bot] Claude's fix is identical to the current file "
                "(after stripping whitespace) – skipping PR creation."
            )
        else:
            try:
                branch_name = (
