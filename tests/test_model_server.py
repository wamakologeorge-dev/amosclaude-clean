...
def test_context_window_preserves_prefix_and_tail() -> None:
    ...
    assert window.trim_token_ids(list(range(12)), keep_prefix=2) == [0, 1, 6, 7, 8, 9, 10, 11]
...
def test_kv_cache_appends_sequence_dimension() -> None:
    ...
    key2, value2 = apply_kv_cache(second, second, (first, first))
...
def test_sampling_returns_valid_token() -> None:
    ...
    token = sample_next_token(logits, SamplingConfig(temperature=1.0, top_p=0.9, do_sample=False))
...
def test_stop_evaluator_detects_eos_and_sequence() -> None:
    ...
    assert stopped and reason == "stop_sequence:```"
...
def test_multi_head_attention_shapes_and_cache() -> None:
    ...
    assert cache is not None and cache.length == 3
    output2, cache2 = layer(torch.randn(2, 1, 16), cache=cache, use_cache=True)
    assert output2.shape == (2, 1, 16)
    assert cache2 is not None and cache2.length == 4


def test_transformer_stack_forward() -> None:
    model = TransformerStack(TransformerConfig(vocab_size=257, d_model=32, num_heads=4, num_layers=2, max_length=64))
    hidden, caches = model(torch.randint(0, 257, (2, 5)), use_cache=True)
    assert hidden.shape == (2, 5, 32)
    assert len(caches) == 2
    assert all(cache is not None and cache.length == 5 for cache in caches)
