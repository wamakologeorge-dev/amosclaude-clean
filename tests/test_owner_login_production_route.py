...
def test_railway_starts_owner_enabled_application() -> None:
    ...
    assert "uvicorn amoscloud_ai.owner_app:app" in railway
...
def test_owner_application_exposes_root_and_legacy_api_paths() -> None:
    ...
    assert "app.include_router(owner_access_gateway.router)" in owner_app
    assert 'prefix="/api/v1"' in owner_app
