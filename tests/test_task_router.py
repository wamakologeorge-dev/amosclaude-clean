...
def _account(monkeypatch, tmp_path):
    ...
    with auth._connect() as db:
        cursor = db.execute(
            ...
            ("Router User", "router@example.com", "password", datetime.now(timezone.utc).isoformat()),
        )
        user_id = int(cursor.lastrowid)
        db.commit()
        agent_tokens.credit_tokens(db, user_id, 100, reason="test_credit", reference="test-credit-1")
    monkeypatch.setattr(task_router, "get_user_from_session", lambda _token: {"id": user_id, "name": "Router User"})
    return user_id
...
def test_task_approval_runner_claim_and_completion(monkeypatch, tmp_path):
    ...
    approved = request("POST", f"/api/v1/tasks/{task_id}/approve", cookies={"amos_session": "test"})
...
def test_cancel_refunds_reserved_credits(monkeypatch, tmp_path):
    ...
    created = request(
        "POST",
        "/api/v1/tasks",
        json={"objective": "Review this project", "mode": "review", "require_approval": False},
        cookies={"amos_session": "test"},
    )
    task_id = created.json()["id"]
    cancelled = request(
