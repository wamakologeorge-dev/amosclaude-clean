...
def test_platform_presents_one_truthful_engineering_operator() -> None:
    ...
    assert "Repository operations use Amosclaud authorization" in html
    assert "reports the exact blocker instead of pretending" in html
...
def test_native_executor_never_substitutes_platform_source() -> None:
    ...
    assert "AMOSCLAUD_MODEL_URL or OLLAMA_URL" in source
