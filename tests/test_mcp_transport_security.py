...
def test_public_mcp_transport_security_allows_amosclaud_and_chatgpt_hosts(monkeypatch):
    ...
    assert "amosclauds.com" in settings.allowed_hosts
    assert "www.amosclaud.com" in settings.allowed_hosts
    assert "https://chatgpt.com" in settings.allowed_origins
    assert "https://chat.openai.com" in settings.allowed_origins
