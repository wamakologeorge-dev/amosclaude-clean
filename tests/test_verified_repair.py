...
def test_runtime_selects_changed_file_compilation_and_focused_test(
    tmp_path: Path,
) -> None:
    (tmp_path / "src").mkdir()
...
def test_runtime_adds_diff_integrity_for_real_git_workspace(tmp_path: Path) -> None:
    (tmp_path / ".git").mkdir()
...
def test_model_json_parser_accepts_fenced_object() -> None:
...
def test_verified_repair_workflow_wires_ollama_and_changed_file_checks() -> None:
    ...
    assert old_test_list not in workflow
