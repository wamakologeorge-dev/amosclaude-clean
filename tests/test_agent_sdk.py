...
def test_sdk_connects_to_hosted_agent_with_bearer_key(monkeypatch):
    ...
    def open_request(request, timeout):
        captured.update(url=request.full_url, authorization=request.headers["Authorization"], payload=json.loads(request.data), timeout=timeout)
        return Response({"accepted": True, "pipeline_id": "pipeline-1"})
    monkeypatch.setattr("amosclaud_agent_sdk.client.urlopen", open_request)
    result = AmosclaudAgentClient(api_key="amos_aut_test", timeout=12).run("Inspect repository", mode="build")
...
def test_admin_session_does_not_need_api_key(monkeypatch):
    captured = {}
    def open_request(request, timeout):
        captured.update(cookie=request.headers["Cookie"], authorization=request.headers.get("Authorization"))
...
def test_build_script_help_never_runs_an_installer():
    result = subprocess.run([sys.executable, "scripts/build_wheel.py", "--help"], text=True, capture_output=True, check=True)
